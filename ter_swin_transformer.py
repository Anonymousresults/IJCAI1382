...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = QuantizeLinear(in_features, hidden_features, weight_bits=2)
        self.norm1 = nn.LayerNorm(hidden_features)
        self.act = nn.GELU()

        self.fc2 = QuantizeLinear(hidden_features, out_features, weight_bits=2)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = QuantizeLinear(dim, 3 * dim, weight_bits=2, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = QuantizeLinear(dim, dim,weight_bits=2)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = QuantizeLinear(4 * dim, 2 * dim, bias=False, weight_bits=2)
...
@register_model
def ter_swin_small(pretrained=False, **kwargs):
    """ Swin-S @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('swin_small_patch4_window7_224', pretrained=False, **model_kwargs)


@register_model
def ter_swin_tiny(pretrained=False, **kwargs):
